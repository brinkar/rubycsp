...
*"CSP (Communicating Sequential Processes)":http://en.wikipedia.org/wiki/CSP library for Ruby*
...
Well, enough of what it doesn't do. The library has been influenced by the PyCSP and JCSP libraries with some other constructs introduced to make it more Ruby-ish. You can check out the syntax and behaviour of the library by looking at (and running) the files in the examples/ and tests/ directories. Later on, a more formal tutorial, along with some API-docs, might be constructed.
...
* Alternative class. Reading from one of many channels depending on availaility of data.
